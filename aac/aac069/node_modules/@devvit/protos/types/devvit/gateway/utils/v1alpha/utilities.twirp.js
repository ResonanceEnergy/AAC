...
import { CacheContextActionsRequest, PurgeResult, AccessKeyPurgeRequest, InstallationsPurgeRequest, InstallationPurgeRequest, AppVersionPurgeRequest, AppPurgeRequest, AppVersionPublishStatusPurgeRequest, PurgeAllRequest, AcceptInviteRequest, AcceptInviteResponse, } from './utilities.js';
...
(function (CacheUtilsMethod) {
    ...
    CacheUtilsMethod["PurgeAppVersionPublishStatus"] = "PurgeAppVersionPublishStatus";
    CacheUtilsMethod["PurgeAll"] = "PurgeAll";
...
export const CacheUtilsMethodList = [
    ...
    CacheUtilsMethod.PurgeAppVersionPublishStatus,
    CacheUtilsMethod.PurgeAll,
];
...
function matchCacheUtilsRoute(method, events) {
    switch (method) {
        ...
        case 'PurgeAppVersionPublishStatus':
            return async (ctx, service, data, interceptors) => {
                ctx = { ...ctx, methodName: 'PurgeAppVersionPublishStatus' };
                await events.onMatch(ctx);
                return handleCacheUtilsPurgeAppVersionPublishStatusRequest(ctx, service, data, interceptors);
            };
        case 'PurgeAll':
            return async (ctx, service, data, interceptors) => {
                ctx = { ...ctx, methodName: 'PurgeAll' };
                await events.onMatch(ctx);
                return handleCacheUtilsPurgeAllRequest(ctx, service, data, interceptors);
            };
        default:
            events.onNotFound();
...
function handleCacheUtilsPurgeAppVersionPublishStatusRequest(ctx, service, data, interceptors) {
    switch (ctx.contentType) {
        case TwirpContentType.JSON:
            return handleCacheUtilsPurgeAppVersionPublishStatusJSON(ctx, service, data, interceptors);
        case TwirpContentType.Protobuf:
            return handleCacheUtilsPurgeAppVersionPublishStatusProtobuf(ctx, service, data, interceptors);
        default:
            const msg = 'unexpected Content-Type';
            throw new TwirpError(TwirpErrorCode.BadRoute, msg);
    }
}
function handleCacheUtilsPurgeAllRequest(ctx, service, data, interceptors) {
    switch (ctx.contentType) {
        case TwirpContentType.JSON:
            return handleCacheUtilsPurgeAllJSON(ctx, service, data, interceptors);
        case TwirpContentType.Protobuf:
            return handleCacheUtilsPurgeAllProtobuf(ctx, service, data, interceptors);
        default:
            const msg = 'unexpected Content-Type';
            throw new TwirpError(TwirpErrorCode.BadRoute, msg);
    }
...
async function handleCacheUtilsPurgeAppVersionPublishStatusJSON(ctx, service, data, interceptors) {
    let request;
    let response;
    try {
        const body = JSON.parse(data.toString() || '{}');
        request = AppVersionPublishStatusPurgeRequest.fromJSON(body);
    }
    catch (e) {
        if (e instanceof Error) {
            const msg = 'the json request could not be decoded';
    ...
    if (interceptors && interceptors.length > 0) {
        const interceptor = chainInterceptors(...interceptors);
        response = await interceptor(ctx, request, (ctx, inputReq) => {
            return service.PurgeAppVersionPublishStatus(ctx, inputReq);
        });
    }
    else {
        response = await service.PurgeAppVersionPublishStatus(ctx, request);
    }
    return JSON.stringify(PurgeResult.toJSON(response));
}
async function handleCacheUtilsPurgeAllJSON(ctx, service, data, interceptors) {
    let request;
    let response;
    try {
        const body = JSON.parse(data.toString() || '{}');
        request = PurgeAllRequest.fromJSON(body);
    }
    catch (e) {
        if (e instanceof Error) {
            const msg = 'the json request could not be decoded';
    ...
    if (interceptors && interceptors.length > 0) {
        const interceptor = chainInterceptors(...interceptors);
        response = await interceptor(ctx, request, (ctx, inputReq) => {
            return service.PurgeAll(ctx, inputReq);
        });
    }
    else {
        response = await service.PurgeAll(ctx, request);
    }
    return JSON.stringify(PurgeResult.toJSON(response));
}
...
async function handleCacheUtilsPurgeAppVersionPublishStatusProtobuf(ctx, service, data, interceptors) {
    let request;
    let response;
    try {
        request = AppVersionPublishStatusPurgeRequest.decode(data);
    }
    catch (e) {
        if (e instanceof Error) {
            const msg = 'the protobuf request could not be decoded';
...
async function handleCacheUtilsPurgeAllProtobuf(ctx, service, data, interceptors) {
    let request;
    let response;
    try {
        request = PurgeAllRequest.decode(data);
    }
    catch (e) {
        if (e instanceof Error) {
            const msg = 'the protobuf request could not be decoded';
