...
export const shouldSuppressWarning = (warning) => {
    return warning.code === 'EVAL' && (warning.id ?? '').includes('node_modules/@protobufjs/inquire');
};
export const shouldCheckClientForServerImports = (environmentName) => {
 ...
 */
export function readDevvitConfig(repoRoot) {
    ...
    try {
        ...
        throw new Error(`${chalk.red('✖')} ${chalk.bold('devvit plugin error')}\n\n` +
            `Could not read ${chalk.cyan('devvit.json')}. Ensure it exists in the project root and contains valid JSON.\n`);
    }
}
/**
...
export function resolveClientRoot(repoRoot) {
 ...
 * preference.
 */
export function resolvePublicDir(repoRoot, clientRoot) {
    ...
    if (repoPublicExists && clientPublicExists) {
        throw new Error(`${chalk.red('✖')} ${chalk.bold('devvit plugin error')}\n\n` +
            `Found both ${chalk.cyan(path.relative(repoRoot, repoPublicDir))} and ${chalk.cyan(path.relative(repoRoot, clientPublicDir))}.\n` +
            `Choose a single public directory and remove the other.\n`);
    }
    if (repoPublicExists)
        return repoPublicDir;
    if (clientPublicExists)
        return clientPublicDir;
    return undefined;
}
export function getClientInputs(config, repoRoot, clientRoot) {
 ...
 */
export function getServerEntry(repoRoot) {
