...
def find_best_python():
    ...
    if not valid:
        # Show what we found
        logger.info(str(_red("  [X] No suitable Python found!")))
        print(f"      Need Python {REQUIRED_PYTHON[0]}.{REQUIRED_PYTHON[1]} – "
              f"{MAX_PYTHON[0]}.{MAX_PYTHON[1]}")
        if candidates:
            logger.info("      Found (but unsuitable):")
            for p, v in candidates:
                logger.info(f"        {p} → Python {v[0]}.{v[1]}.{v[2]}")
        return None
...
def create_venv(python_path):
    ...
    logger.info(str(_cyan(f"  Creating .venv using {python_path}...")))
    result = subprocess.run(
        [python_path, "-m", "venv", str(VENV_DIR)],
        capture_output=True, text=True, timeout=120,
    )
    if result.returncode != 0:
        logger.info(str(_red(f"  [X] Failed to create venv: {result.stderr}")))
        return False
    ...
    logger.info(str(_cyan("  Upgrading pip...")))
    subprocess.run([pip, "install", "--upgrade", "pip"], capture_output=True, timeout=120)
    logger.info(str(_green("  [+] Virtual environment created")))
    return True
...
def install_deps():
    ...
    logger.info(str(_cyan(f"  Installing dependencies from {REQUIREMENTS.name}...")))
    result = subprocess.run(
        [pip, "install", "-r", str(REQUIREMENTS)],
        capture_output=True, text=True, timeout=600,
    )
    if result.returncode != 0:
        logger.info(str(_red(f"  [X] Dependency installation failed!")))
        # Show which packages failed
        for line in result.stderr.split('\n'):
            if 'error' in line.lower() or 'failed' in line.lower():
                logger.info(str(_red(f"      {line.strip()}")))
        return False

    # Also install the project itself in editable mode
    logger.info(str(_cyan("  Installing AAC package (editable)...")))
    subprocess.run(
        [pip, "install", "-e", str(PROJECT_ROOT)],
        capture_output=True, text=True, timeout=120,
    )

    logger.info(str(_green("  [+] All dependencies installed")))
    return True


def check_syntax():
    """Quick syntax check on all Python files."""
    logger.info(str(_cyan("  Running syntax check on all .py files...")))
    errors = []
    total = 0
    ...
    if errors:
        logger.info(str(_yellow(f"  [!] {len(errors)} syntax errors in {total} files:")))
        for err in errors[:10]:
            logger.info(str(_red(f"      {err}")))
        if len(errors) > 10:
            logger.info(str(_yellow(f"      ... and {len(errors) - 10} more")))
    else:
        logger.info(str(_green(f"  [+] All {total} .py files pass syntax check")))
...
def check_critical_imports():
    ...
    logger.info(str(_cyan(f"  Testing {len(critical_packages)} critical imports...")))
    passed = []
    failed = []
    ...
    if failed:
        logger.info(str(_yellow(f"  [!] {len(failed)} imports failed:")))
        for pkg, err in failed:
            logger.info(str(_red(f"      {pkg}: {err}")))
    else:
        logger.info(str(_green(f"  [+] All {len(passed)} critical imports work")))
...
def check_env_file():
    ...
    if env_file.exists():
        logger.info(str(_green("  [+] .env file exists")))
        return True
    elif template.exists():
        shutil.copy2(template, env_file)
        logger.info(str(_yellow("  [!] Created .env from .env.template — fill in your API keys")))
        return True
    else:
        logger.info(str(_yellow("  [!] No .env or .env.template found — some features may not work")))
        return False
...
def print_report(machine_info, venv_info, syntax_errors, import_results, state):
    """Print final report."""
    logger.info("")
    logger.info(str(_bold("=" * 60)))
    logger.info(str(_bold(f"  MACHINE SETUP REPORT — {machine_info['machine_name']}")))
    logger.info(str(_bold("=" * 60)))
    logger.info("")
    logger.info(f"  Machine:    {machine_info['machine_name']} ({machine_info['hostname']})")
    logger.info(f"  OS:         {machine_info['os']} {machine_info['os_version'][:30]}")
    logger.info(f"  User:       {machine_info['user']}")
    logger.info(f"  Project:    AAC v{state['project_version']}")
    logger.info(f"  Workspace:  {PROJECT_ROOT}")
    logger.info("")

    if venv_info.get("ok"):
        v = venv_info["version"]
        logger.info(str(_green(f"  Python:     {v[0]}.{v[1]}.{v[2]} (OK)")))
    else:
        logger.info(str(_red(f"  Python:     ISSUE — see above")))

    logger.info(f"  Syntax:     {len(syntax_errors)} errors")

    if import_results:
        p, f = import_results
        if f:
            logger.info(str(_yellow(f"  Imports:    {len(p)} passed, {len(f)} failed")))
        else:
            logger.info(str(_green(f"  Imports:    {len(p)} passed, 0 failed")))
    logger.info("")

    if not syntax_errors and import_results and not import_results[1]:
        logger.info(str(_green("  ✅  MACHINE IS READY — united front confirmed")))
    elif syntax_errors:
        logger.info(str(_yellow("  ⚠️  Fix syntax errors first: python setup_machine.py --fix")))
    else:
        logger.info(str(_yellow("  ⚠️  Some imports failed — check dependencies")))

    logger.info("")
    logger.info(f"  State saved to: .machine_state.json")
    logger.info(f"  To compare machines: diff QUSAR/.machine_state.json QFORGE/.machine_state.json")
    logger.info("")
...
def main():
    ...
    logger.info(str(_bold("  Step 1: Machine Detection")))
    machine_info = detect_machine()
    logger.info(str(_green(f"  [+] Machine: {machine_info['machine_name']}")))
    logger.info(str(_green(f"  [+] OS: {machine_info['os']} | Arch: {machine_info['arch']}")))
    logger.info("")

    # ── Step 2: Find Python ─────────────────────────────────────
    logger.info(str(_bold("  Step 2: Python Discovery")))
    best_python = find_best_python()
    if best_python:
        best_ver = _get_python_version(best_python)
        logger.info(str(_green(f"  [+] Best Python: {best_python}")))
        logger.info(str(_green(f"  [+] Version: {best_ver[0]}.{best_ver[1]}.{best_ver[2]}")))
    else:
        logger.info(str(_red("  [X] No suitable Python (3.9–3.13) found!")))
        logger.info(str(_red("      Install Python 3.12 from https://python.org/downloads/")))
        sys.exit(1)
    logger.info("")

    # ── Step 3: Virtual Environment ─────────────────────────────
    logger.info(str(_bold("  Step 3: Virtual Environment")))
    venv_info = check_venv()

    if args.reinstall and venv_info["exists"]:
        logger.info(str(_yellow("  [!] Removing existing venv for reinstall...")))
        shutil.rmtree(VENV_DIR)
        venv_info = {"exists": False}

    if not venv_info["exists"]:
        if args.check:
            logger.info(str(_yellow("  [!] No .venv — run without --check to create")))
        else:
            create_venv(best_python)
            venv_info = check_venv()
    elif not venv_info.get("ok"):
        logger.info(str(_yellow(f"  [!] venv Python version {venv_info.get('version')} is not ideal")))
        if not args.check:
            logger.info(str(_yellow("  [!] Recreating with better Python...")))
            shutil.rmtree(VENV_DIR)
            create_venv(best_python)
            venv_info = check_venv()
    else:
        v = venv_info["version"]
        logger.info(str(_green(f"  [+] .venv exists — Python {v[0]}.{v[1]}.{v[2]}")))
    logger.info("")

    # ── Step 4: Dependencies ────────────────────────────────────
    logger.info(str(_bold("  Step 4: Dependencies")))
    if venv_info.get("exists") and not args.check:
        install_deps()
    elif args.check:
        logger.info(str(_yellow("  [!] Skipping install (--check mode)")))
    logger.info("")

    # ── Step 5: Environment file ────────────────────────────────
    logger.info(str(_bold("  Step 5: Environment")))
    check_env_file()
    logger.info("")

    # ── Step 6: Syntax check ────────────────────────────────────
    logger.info(str(_bold("  Step 6: Syntax Validation")))
    syntax_errors = check_syntax()
    logger.info("")

    # ── Step 7: Import check ────────────────────────────────────
    logger.info(str(_bold("  Step 7: Import Validation")))
    import_results = None
    if venv_info.get("exists") and venv_info.get("ok"):
        import_results = check_critical_imports()
    else:
        logger.info(str(_yellow("  [!] Skipping import check — venv not ready")))
    logger.info("")
