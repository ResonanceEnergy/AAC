#!/usr/bin/env python3
r"""
AAC War Room — BUY XLE CALL (March 27, 2026)
=============================================
XLE $65 Call, June 18 2026 expiry, limit $2.68

IBKR live port 7496, account U24346218.

Usage:
  .venv\Scripts\python.exe _place_xle_call.py              # DRY RUN (default)
  .venv\Scripts\python.exe _place_xle_call.py --live        # LIVE execution
  .venv\Scripts\python.exe _place_xle_call.py --price-only  # Just scan price
"""
...
SIDE        = "buy"
LIMIT_PRICE = 2.68
QUANTITY    = 3            # 3 contracts = $804 premium
...
async def main():
    ...
    print("=" * 70)
    print(f"  AAC WAR ROOM -- BUY XLE CALL")
    ...
    print(f"\n  Order:   BUY {QUANTITY}x XLE ${STRIKE:.0f}C  exp {EXPIRY[:4]}-{EXPIRY[4:6]}-{EXPIRY[6:]}")
    ...
    # Show existing XLE positions
    portfolio = ib.portfolio()
    xle_positions = [p for p in portfolio if p.contract.symbol == SYMBOL]
    if xle_positions:
        print(f"\n  EXISTING {SYMBOL} POSITIONS:")
    ...
    if not qualified:
        print(f"\n  ERROR: Contract {SYMBOL} ${STRIKE:.0f}C {EXPIRY} NOT FOUND in IBKR chain.")
    ...
    if mode == "dry":
        print(f"\n  DRY RUN -- Would place: BUY {QUANTITY}x {SYMBOL} ${STRIKE:.0f}C {EXPIRY} @ ${LIMIT_PRICE:.2f}")
    ...
    # =========================================================================
    print(f"\n{'=' * 70}")
    print(f"  EXECUTING: BUY {QUANTITY}x {SYMBOL} ${STRIKE:.0f}C {EXPIRY} @ ${LIMIT_PRICE:.2f}")
    print(f"{'=' * 70}\n")
