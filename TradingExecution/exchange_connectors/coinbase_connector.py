...
class CoinbaseConnector(BaseExchangeConnector):
    ...
    def __init__(
        ...
        if not api_key:
            ...
            self.api_secret = config.coinbase.api_secret
    ...
    @with_circuit_breaker("coinbase_ticker", failure_threshold=5, timeout=30.0)
    ...
    @with_circuit_breaker("coinbase_orderbook", failure_threshold=5, timeout=30.0)
    ...
    async def get_trade_fee(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                'maker': fees.get('maker', 0.005),
                'taker': fees.get('taker', 0.005),
            }
        except Exception as e:
    ...
    async def create_stop_loss_order(
        ...
        try:
            params = {
                ...
                'stop': 'loss',  # Coinbase uses 'loss' or 'entry'
            }
            if client_order_id:
    ...
    def _check_credentials(self) -> bool:
        """Check if API credentials are configured (including passphrase)"""
