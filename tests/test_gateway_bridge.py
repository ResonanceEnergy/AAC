...
class TestGatewayBridge:
    ...
    def test_bridge_has_classifier(self):
        bridge = OpenClawGatewayBridge()
        # Intent classification is handled by AACIntentClassifier (static class)
        assert AACIntentClassifier is not None
        assert hasattr(AACIntentClassifier, 'classify')
